...
internal object TrackSettingsAtomicIo {

    private const val TAG = "TrackSettingsAtomicIo"
    private const val CONFIG_DIR_NAME = "Config"
    private const val FILE_NAME = "track_settings.json"
    ...
    private fun writeFileAtomic(storage: Storage.FileStorage, rawJson: String): Boolean {
        ...
        val target = storage.settingsFile
    ...
    private fun writeSafAtomic(context: Context, storage: Storage.SafStorage, rawJson: String): Boolean {
        ...
        return runCatching {
            val tmpName = "track_settings.tmp.${System.currentTimeMillis()}"
            ...
            if (target != null) {
                backup = findFileIgnoreCase(dir, "$FILE_NAME.bak")
                if (backup != null) {
                    runCatching { backup!!.delete() }
                }
                backupRenamed = runCatching { target.renameTo("$FILE_NAME.bak") }.getOrDefault(false)
            }
    ...
    private fun ensureFileInitialized(storage: Storage.FileStorage): Boolean {
        ...
        return runCatching {
            target.writeText(TrackSettingsState.empty().toJson().toString(2), Charsets.UTF_8)
            true
        }.getOrElse {
    ...
    private fun ensureSafInitialized(context: Context, storage: Storage.SafStorage): Boolean {
        ...
        val raw = TrackSettingsState.empty().toJson().toString(2)
        return runCatching {
