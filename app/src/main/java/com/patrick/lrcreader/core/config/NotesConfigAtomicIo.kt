...
internal object NotesConfigAtomicIo {

    private const val TAG = "NotesConfigAtomicIo"
    ...
    private fun ensureFileInitialized(storage: Storage.FileStorage): Boolean {
        ...
        return runCatching {
            target.writeText(NotesConfigState.empty().toJson().toString(2), Charsets.UTF_8)
            true
        }.getOrElse {
    ...
    private fun ensureSafInitialized(context: Context, storage: Storage.SafStorage): Boolean {
        ...
        val raw = NotesConfigState.empty().toJson().toString(2)
        return runCatching {
